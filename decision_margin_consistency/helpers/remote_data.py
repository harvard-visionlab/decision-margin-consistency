...
def decompress_tarfile_if_needed(file_path, output_dir=None):
    ...
    # Extract hash ID from filename using regex
    filename = os.path.basename(file_path)
    match = HASH_REGEX.search(filename)
    if match:
        hash_id = match.group(1)  # Use the hash ID found in the filename
    else:
        # No hash ID in the filename, compute the hash ID of the tar file
        hash_id = calculate_sha256(file_path)[:8]  # Use the first 8 characters of the hash for the folder name

    # Set the output directory. If none is provided, default to a new folder named with the hash ID
    if output_dir is None:
        output_dir = os.path.join(dir_name, hash_id)
